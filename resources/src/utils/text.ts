...
import { SingleFrameAssets } from './frameAssets'

const margin = 6

const createText = (opts: {
  text: string; size: number; color?: number; outlineColor?: number
}): Container => {
  ...
  text.filters = []
  text.filters.push(new OutlineFilter(opts.size / 8, opts.outlineColor ?? 0xffffff, 1))
  text.localTransform
...
export const createLabel = (opts: {
  name: string; text: string; size: number; color?: number; scaleX?: number;
}): SingleFrameAssets => {
